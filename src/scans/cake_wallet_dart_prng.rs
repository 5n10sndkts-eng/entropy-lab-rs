...
pub fn run() -> Result<()> {
    ...
    println!("Time range: {} to {} ({} seconds)", start_sec, end_sec, total_seconds);
    println!("Sampling 5 microsecond offsets per second = {} total iterations", total_seconds * 5);
    ...
    for timestamp_sec in start_sec..=end_sec {
        for &micro_offset in &micro_offsets {
            ...
            if batch_timestamps.len() >= batch_size {
                ...
                eprintln!("Progress: {}/{} seconds ({:.2}%) - Speed: {:.2} M/s - Found: {}", 
                    timestamp_sec - start_sec, total_seconds,
