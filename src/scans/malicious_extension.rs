...
pub fn run() -> Result<()> {
    ...
    println!("Desired Poison Address: {}...{}", target_prefix, target_suffix);
    ...
    // Run a few batches
    for i in 0..100 {
        let results = solver.compute_address_poisoning(seed_base, batch_size, target_prefix, target_suffix)?;
        ...
        if !results.is_empty() {
            for &seed in &results {
                ...
                if addr_str.starts_with(target_prefix) && addr_str.ends_with(target_suffix) {
                    ...
                    println!("  Match:       {}^^^^^...^^^^^", " ".repeat(target_prefix.len()));
