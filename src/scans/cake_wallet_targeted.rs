...
pub fn run_targeted() -> Result<()> {
    ...
    println!("Loaded {} vulnerable mnemonic hashes", vulnerable_hashes.len());
    ...
    for seed_index in 0..1_048_576 {
        ...
        if let Ok(mnemonic) = Mnemonic::from_entropy(&entropy[0..16]) {
            ...
            if vulnerable_hashes.contains(&hash_hex) {
                found += 1;
                eprintln!("[VULNERABLE] Seed {}: {} -> Hash: {}", 
                    seed_index, &mnemonic_str[..50], &hash_hex[..16]);
        ...
        if checked % 100_000 == 0 {
            eprintln!("Progress: {}/1048576 ({:.1}%) - Found: {}", 
                checked, (checked as f64 / 1_048_576.0) * 100.0, found);
        }
