...
fn run_with_target(target: &str, start_ts: u32, end_ts: u32, multipath: bool) -> Result<()> {
    #[cfg(feature = "gpu")]
    {
        ...
        if multipath {
            let results =
                solver.compute_milk_sad_crack_multipath(start_ts, end_ts, &target_hash160)?;
            if !results.is_empty() {
                info!(
                ...
                for (timestamp, addr_idx) in results {
                    ...
                    let derived_address = generate_address_from_entropy_vec(
                        &entropy,
                        addr_idx,
                        AddressType::P2PKH,
                        false,
            ...
            if !results.is_empty() {
                ...
                for timestamp in results {
                    ...
                    let derived_address =
                        generate_address_from_entropy_vec(&entropy, 0, AddressType::P2PKH, false);
