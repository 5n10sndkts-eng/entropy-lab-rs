...
pub fn run() -> Result<()> {
    println!("Milk Sad Vulnerability Scanner (100% GPU)");
    println!("This scanner requires a target address. Use: milk-sad --target <address>");
    println!("For address generation mode, use the original hybrid implementation.");
    
    Ok(())
}

pub fn run_with_target(target: String, start_ts_opt: Option<u32>, end_ts_opt: Option<u32>, multipath: bool) -> Result<()> {
    println!("Milk Sad Vulnerability Scanner (100% GPU)");
    if multipath {
        println!("Mode: Multi-Path (Checking 30 addresses per timestamp)");
    } else {
        println!("Mode: Single-Path (Checking m/44'/0'/0'/0/0 only)");
    }
        ...
        if !results.is_empty() {
            ...
            for (timestamp, addr_idx) in results {
                ...
                if derived_address == target {
                    ...
                    println!("Mnemonic: {}", Mnemonic::from_entropy(&entropy).unwrap());
                } else {
                    println!("[FALSE POSITIVE] Timestamp {} (Index {}) -> {}", timestamp, addr_idx, derived_address);
                }
            }
        } else {
        ...
        if !results.is_empty() {
            ...
            for timestamp in results {
                ...
                if derived_address == target {
                    ...
                    println!("[FALSE POSITIVE] Timestamp {} -> {}", timestamp, derived_address);
                }
            }
        } else {
            println!("\nScan complete. No match found.");
