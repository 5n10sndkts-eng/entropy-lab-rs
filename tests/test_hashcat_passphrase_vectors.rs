...
fn test_hashcat_passphrase_uncompressed() {
    ...
    assert_eq!(uncompressed_bytes.len(), 65, "Uncompressed pubkey should be 65 bytes");
    assert_eq!(uncompressed_bytes[0], 0x04, "Uncompressed pubkey should start with 0x04");
    ...
    println!("  4. P2PKH Address (uncompressed): {}", address_uncompressed);
...
fn test_hashcat_passphrase_compressed() {
    ...
    assert_eq!(compressed_bytes.len(), 33, "Compressed pubkey should be 33 bytes");
    assert!(
...
fn test_hashcat_passphrase_all_formats() {
    ...
    // All assertions
    assert_eq!(privkey_hex, "127e6fbfe24a750e72930c220a8e138275656b8e5d8f48a98c3c92df2caba935");
